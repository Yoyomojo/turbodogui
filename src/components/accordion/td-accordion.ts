...
import styles from "./td-accordion.css?raw";
export class UIAccordion extends UIComponent {
    private items: Map<number, {
        header: HTMLElement;
        content: HTMLElement;
    }> = new Map();
    ...
    private attachEventListeners(allowMultiple: boolean): void {
        ...
        headers.forEach((header) => {
            ...
            const keydownHandler = (e: KeyboardEvent) => {
                ...
                if (e.key === "ArrowDown") {
                    e.preventDefault();
                    const nextHeader = this.root.querySelector<HTMLButtonElement>(`[data-accordion-index="${index + 1}"]`);
                    nextHeader?.focus();
                }
                else if (e.key === "ArrowUp") {
                    e.preventDefault();
                    const prevHeader = this.root.querySelector<HTMLButtonElement>(`[data-accordion-index="${index - 1}"]`);
                    prevHeader?.focus();
                }
                else if (e.key === "Home") {
                    e.preventDefault();
                    const firstHeader = this.root.querySelector<HTMLButtonElement>('[data-accordion-index="0"]');
                    firstHeader?.focus();
                }
                else if (e.key === "End") {
                    e.preventDefault();
                    const lastHeader = this.root.querySelector<HTMLButtonElement>(`[data-accordion-index="${this.root.querySelectorAll('[data-accordion-index]').length - 1}"]`);
                    lastHeader?.focus();
                }
            };
    ...
    private toggleItem(index: number, allowMultiple: boolean): void {
        const header = this.root.querySelector<HTMLButtonElement>(`[data-accordion-index="${index}"]`);
        const content = this.root.querySelector<HTMLDivElement>(`[data-accordion-index="${index}"][role="tabpanel"]`);
        if (!header || !content)
            return;
        const isActive = header.classList.contains("active");
        if (!allowMultiple && this.activeIndex !== null && this.activeIndex !== index) {
            const prevHeader = this.root.querySelector<HTMLButtonElement>(`[data-accordion-index="${this.activeIndex}"]`);
            const prevContent = this.root.querySelector<HTMLDivElement>(`[data-accordion-index="${this.activeIndex}"][role="tabpanel"]`);
...
customElements.define("td-accordion", UIAccordion);
