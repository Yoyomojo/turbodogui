...
import styles from "./td-table.css?raw";
...
export class UITable extends UIComponent {
    ...
    connectedCallback(): void {
        const hasConfig = this._columns.length > 0 ||
            this._data.length > 0 ||
    ...
    private getLeafColumns(columns: TableColumn[]): TableColumn[] {
        return columns.flatMap((col) => col.children && col.children.length > 0 ? this.getLeafColumns(col.children) : [col]);
    }
    ...
    private getMaxDepth(columns: TableColumn[], depth = 1): number {
        return Math.max(...columns.map((col) => col.children && col.children.length > 0
        ...
        return levels
            ...
            const cells = levelCols
                ...
                const stickyStyle = stickyHeader
                    ? ` style="top: calc(${depth} * var(--td-table-row-height, 45px)); z-index: ${maxDepth - depth + 1};"`
                    : "";
                if (!isLeaf) {
    ...
    protected render(): void {
        ...
        if (this._columns.length === 0 &&
            ...
            const scrollStyle = maxRows > 0
                ? ` style="max-height: calc(${maxRows + 1} * var(--td-table-row-height, 45px)); overflow-y: auto;"`
                : "";
            this.root.innerHTML = `
        ${this.css(styles)}
      ...
      `;
        ...
        const scrollStyle = maxRows > 0
            ? ` style="max-height: calc(${maxRows + headerDepth} * var(--td-table-row-height, 45px)); overflow-y: auto;"`
            : "";
        const headerRows = this.buildHeaderRows(columns, stickyHeader);
        const rows = data
            .map((row, rowIdx) => {
            const rowVariant = row["_variant"] as string | undefined;
            const rowClass = rowVariant ? ` class="row--${this.escapeHtml(rowVariant)}"` : "";
            const cells = leafColumns
                .map((col) => {
                const cellVariant = row[`_variant_${col.key}`] as string | undefined;
                const cellClass = cellVariant ? ` class="cell--${this.escapeHtml(cellVariant)}"` : "";
                if (col.render) {
                    return `<td${cellClass} data-row-index="${rowIdx}" data-col-key="${this.escapeHtml(col.key)}">${col.render(row[col.key], row)}</td>`;
                }
                return `<td${cellClass}>${this.escapeHtml(row[col.key])}</td>`;
            })
                .join("");
    ...
    private attachListeners(allColumns: TableColumn[], columns: TableColumn[], rawData: RowData[], filename: string): void {
        ...
        if (csvBtn) {
            csvBtn.addEventListener("click", () => this.exportCsv(allColumns, columns, rawData, filename));
        }
    ...
    private exportCsv(allColumns: TableColumn[], leafColumns: TableColumn[], data: RowData[], filename: string): void {
        const headerRows = this.buildCsvHeaderRows(allColumns);
        const csvRows = headerRows.map((row) => row.map((cell) => `"${cell.replace(/"/g, '""')}"`).join(","));
        const dataRows = data.map((row) => leafColumns
...
customElements.define("td-table", UITable);
