...
import styles from './td-pie-chart.css?raw';
...
export class UIPieChart extends UIComponent {
    ...
    protected render() {
        ...
        this._data.forEach((slice, i) => {
            ...
            svgParts.push(`<path d="M${cx},${cy} L${x1},${y1} A${radius},${radius} 0 ${largeArc} 1 ${x2},${y2} Z" fill="${color}" data-index="${i}" tabindex="0" />`);
            if (this._showValues && angle > 5) {
                ...
                svgParts.push(`<text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" class="pie-slice-label">${pct}%</text>`);
            }
            startAngle = endAngle;
        });
        svgParts.push('</svg>');
