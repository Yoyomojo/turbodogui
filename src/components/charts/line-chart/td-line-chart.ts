...
import styles from './td-line-chart.css?raw';
...
export class UILineChart extends UIComponent {
    ...
    attributeChangedCallback(name: string, _oldValue: string, newValue: string) {
        ...
        if (name === 'x-labels') {
            try {
                this._xLabels = JSON.parse(newValue);
            }
            catch {
                this._xLabels = [];
            }
    ...
    connectedCallback() {
        ...
        if (this.hasAttribute('x-labels')) {
            try {
                this._xLabels = JSON.parse(this.getAttribute('x-labels')!);
            }
            catch {
                this._xLabels = [];
            }
    ...
    protected render() {
        ...
        const xPos = (i: number) => pointCount <= 1
            ? chartLeft + chartWidth / 2
            : chartLeft + (i / (pointCount - 1)) * chartWidth;
        const yPos = (v: number) => chartTop + chartHeight - (v / niceMax) * chartHeight;
