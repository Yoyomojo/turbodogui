...
export class UILineChart extends UIComponent {
  ...
  static observedAttributes = ['data', 'x-labels', 'colors', 'height', 'width', 'title', 'legend-position'];
  ...
  protected render() {
    ...
    let svgHtml = `<svg class="line-chart-svg" viewBox="0 0 ${svgWidth} ${totalHeight}" overflow="visible" style="width:100%;">`;
    ...
    svgHtml += `</svg>`;
    svgHtml += `<div id="${tooltipId}" class="line-tooltip" style="display:none;position:absolute;z-index:10;"></div>`;
    ...
    if (this._legendPosition === 'top') {
      ...
      layout = `${titleHtml}<div style="display:flex;"><div>${legendHtml}</div><div style="flex:1;">${svgHtml}</div></div>`;
    } else {
      layout = `${titleHtml}<div style="display:flex;"><div style="flex:1;">${svgHtml}</div><div>${legendHtml}</div></div>`;
    }
