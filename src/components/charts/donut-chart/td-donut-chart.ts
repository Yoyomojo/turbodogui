...
import styles from './td-donut-chart.css?raw';
...
export class UIDonutChart extends UIComponent {
    ...
    protected render() {
        ...
        this._data.forEach((slice, i) => {
            ...
            const isFullRing = angle >= 359.999;
            const d = isFullRing
                ? [
                    `M ${(cx + outerRadius).toFixed(3)},${cy.toFixed(3)}`,
                    `A ${outerRadius},${outerRadius} 0 1 1 ${(cx - outerRadius).toFixed(3)},${cy.toFixed(3)}`,
                    `A ${outerRadius},${outerRadius} 0 1 1 ${(cx + outerRadius).toFixed(3)},${cy.toFixed(3)}`,
                    `M ${(cx + innerRadius).toFixed(3)},${cy.toFixed(3)}`,
                    `A ${innerRadius},${innerRadius} 0 1 0 ${(cx - innerRadius).toFixed(3)},${cy.toFixed(3)}`,
                    `A ${innerRadius},${innerRadius} 0 1 0 ${(cx + innerRadius).toFixed(3)},${cy.toFixed(3)}`,
                    'Z'
                ].join(' ')
                : [
            ...
            svgParts.push(`<path d="${d}" fill="${color}" fill-rule="evenodd" data-index="${i}" tabindex="0" />`);
            if (this._showValues && angle > 5) {
                ...
                svgParts.push(`<text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" class="donut-slice-label">${pct}%</text>`);
            }
            startAngle = endAngle;
        });
        if (this._centerLabel) {
            svgParts.push(`<text x="${cx}" y="${cy}" text-anchor="middle" dominant-baseline="middle" class="donut-center-label">${this._centerLabel}</text>`);
        }
        svgParts.push('</svg>');
