...
import styles from './td-bar-chart.css?raw';
...
export class UIBarChart extends UIComponent {
    static tag = 'td-bar-chart';
    static observedAttributes = ['data', 'src', 'colors', 'height', 'width', 'horizontal', 'title', 'font-size', 'legend-position', 'reference-lines', 'line-series', 'bar-series', 'categories', 'show-values', 'group-tooltip'];
    ...
    private _groupTooltip: boolean = false;
    ...
    get groupTooltip(): boolean {
        return this._groupTooltip;
    }
    set groupTooltip(val: boolean) {
        this._groupTooltip = val;
        this.render();
    }
    attributeChangedCallback(name: string, oldValue: string, newValue: string) {
        ...
        if (name === 'group-tooltip') {
            this._groupTooltip = newValue !== null && newValue !== 'false';
            this.render();
        }
        if (name === 'src') {
            if (newValue)
    ...
    connectedCallback() {
        ...
        if (this.hasAttribute('group-tooltip')) {
            this._groupTooltip = this.getAttribute('group-tooltip') !== 'false';
        }
    ...
    private _positionTooltip(tooltip: HTMLElement, svg: SVGSVGElement, clientX: number, clientY: number): void {
        const svgBox = svg.getBoundingClientRect();
        const tw = tooltip.offsetWidth || 120;
        const th = tooltip.offsetHeight || 30;
        const offset = 12;
        // Default: right of and above cursor
        let left = clientX - svgBox.left + offset;
        let top = clientY - svgBox.top - th - offset;
        // Flip left if overflowing right edge
        if (left + tw > svgBox.width) {
            left = clientX - svgBox.left - tw - offset;
        }
        // Flip below if overflowing top edge
        if (top < 0) {
            top = clientY - svgBox.top + offset;
        }
        // Hard clamp within chart bounds
        if (left < 0) left = 0;
        if (top < 0) top = 0;
        if (left + tw > svgBox.width) left = svgBox.width - tw;
        if (top + th > svgBox.height) top = svgBox.height - th;
    ...
    private _niceScale(max: number, tickCount = 5): {
        niceMax: number;
        niceStep: number;
    } {
    ...
    protected render() {
        ...
        if (!isGrouped && !this._data.length)
            return;
        if (!isGrouped) {
        ...
        if (svg && tooltip) {
            svg.querySelectorAll<SVGLineElement>('.bar-ref-line-hit').forEach((line) => {
                ...
                line.addEventListener('mousemove', (e) => {
                    this._positionTooltip(tooltip, svg, (e as MouseEvent).clientX, (e as MouseEvent).clientY);
                });
            ...
            svg.querySelectorAll<SVGRectElement>('rect[data-series-index]').forEach((rect) => {
                ...
                const catLabel = this._categories[ci] ?? String(ci + 1);
                const val = series?.values[ci] ?? 0;
                const singleTooltipText = `${series?.label ?? ''}  -  ${catLabel}: ${val}`;
                const getGroupRects = () => svg.querySelectorAll<SVGRectElement>(`rect[data-cat-index="${ci}"]`);
                const highlightGroup = () => getGroupRects().forEach(r => { r.setAttribute('stroke', '#fff'); r.setAttribute('stroke-width', '2'); });
                const unhighlightGroup = () => getGroupRects().forEach(r => { r.removeAttribute('stroke'); r.removeAttribute('stroke-width'); });
                const buildGroupTooltipHtml = () => {
                    const lines = [`<strong>${catLabel}</strong>`];
                    this._barSeries.forEach((s, idx) => {
                        if (this._hiddenBarSeries[idx]) return;
                        const color = s.color || (this._colors.length ? this._colors[idx % this._colors.length] : `hsl(${idx * 60}, 70%, 60%)`);
                        lines.push(`<span style="display:inline-block;width:10px;height:10px;background:${color};border-radius:2px;margin-right:4px;vertical-align:middle;"></span>${s.label}: ${s.values[ci] ?? 0}`);
                    });
                    return lines.join('<br>');
                };
                rect.addEventListener('mouseenter', () => {
                    if (this._groupTooltip) {
                        tooltip.innerHTML = buildGroupTooltipHtml();
                        tooltip.style.whiteSpace = 'normal';
                        highlightGroup();
                    } else {
                        tooltip.textContent = singleTooltipText;
