...
import styles from "./td-carousel.css?raw";
export class UICarousel extends UIComponent {
    ...
    private _countSlides(): number {
        ...
        for (const child of Array.from(this.children)) {
            if (/^slide-\d+$/.test(child.getAttribute("slot") ?? ""))
                count++;
        }
        return count;
    }
    ...
    private _updateView(prevIndex: number): void {
        ...
        slides.forEach(s => s.classList.remove(...ALL));
        outgoing.classList.add("slide--active", "slide--exiting", goingForward ? "slide--exiting-left" : "slide--exiting-right");
        incoming.classList.add("slide--entering", goingForward ? "slide--entering-right" : "slide--entering-left");
        ...
        const nextBtn = this.root.querySelector<HTMLButtonElement>(".arrow--next");
        if (prevBtn) {
            prevBtn.classList.toggle("arrow--hidden", !showPrev);
            prevBtn.disabled = !showPrev;
        }
        if (nextBtn) {
            nextBtn.classList.toggle("arrow--hidden", !showNext);
...
customElements.define("td-carousel", UICarousel);
