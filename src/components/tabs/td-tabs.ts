...
import styles from "./td-tabs.css?raw";
export class UITabs extends UIComponent {
    ...
    private initScrollControls(): void {
        ...
        const list = this.root.querySelector<HTMLElement>(".tabs-list");
        if (list)
            this.resizeObserver.observe(list);
        requestAnimationFrame(() => updateArrows());
    }
    private parseTabsData(): Array<{
        label: string;
    }> {
    ...
    private selectTab(index: number): void {
        ...
        if (activeBtn) {
            activeBtn.scrollIntoView({ block: "nearest", inline: "nearest", behavior: "instant" } as ScrollIntoViewOptions);
            requestAnimationFrame(() => requestAnimationFrame(() => this.scrollHandler?.()));
        }
...
customElements.define("td-tabs", UITabs);
