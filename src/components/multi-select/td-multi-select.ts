...
import styles from "./td-multi-select.css?raw";
...
export class UIMultiSelect extends UIComponent {
    ...
    static get observedAttributes(): string[] {
        return ["label", "options", "value", "search", "placeholder", "search-placeholder", "select-all", "size"];
    }
    ...
    protected render(): void {
        ...
        const size = validSizes.includes(requestedSize as any) ? requestedSize : "medium";
        ...
        const filtered = this._searchQuery.trim()
            ? options.filter((o) => o.label.toLowerCase().includes(this._searchQuery.trim().toLowerCase()))
            : options;
        const filteredSelected = filtered.filter((o) => this._selected.has(o.value));
        const allState = filtered.length > 0 && filteredSelected.length === filtered.length
            ? "all"
...
customElements.define("td-multi-select", UIMultiSelect);
